...
AttinyPing::AttinyPing(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance) {
...
unsigned int AttinyPing::ping_in() {
	unsigned int echoTime = AttinyPing::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return AttinyPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int AttinyPing::ping_cm() {
	unsigned int echoTime = AttinyPing::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return AttinyPingConvert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}


unsigned int AttinyPing::ping_median(uint8_t it) {
...
boolean AttinyPing::ping_trigger() {
#if DISABLE_ONE_PIN != true
	*_triggerMode |= _triggerBit;    // Set trigger pin to output.
#endif
...
void AttinyPing::ping_timer(void (*userFunc)(void)) {
	if (!ping_trigger()) return;         // Trigger a ping, if it returns false, return without starting the echo timer.
	timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
}

 
boolean AttinyPing::check_timer() {
...
void AttinyPing::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void AttinyPing::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
	timer_setup();                       // Configure the timer interrupt.
...
void AttinyPing::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#else
	TIMSK2 &= ~(1<<OCIE2A);
...
void AttinyPing::timer_ms_cntdwn() {
...
unsigned int AttinyPing::convert_in(unsigned int echoTime) {
	return AttinyPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int AttinyPing::convert_cm(unsigned int echoTime) {
	return AttinyPingConvert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}
